(() => {
  // node_modules/@supabase/functions-js/node_modules/tslib/tslib.es6.mjs
...
Option 2: Install and provide the "ws" package:
  ...
  // node_modules/@supabase/auth-js/node_modules/tslib/tslib.es6.mjs
  function __rest(s, e) {
    var t = {};
...
${suffix}`;
  ...
  (() => {
    ...
    function syncInAppProfileForUser(user, { createIfMissing = false } = {}) {
      ...
      const changed = !state.user || localUser.id !== nextUser.id || localUser.name !== nextUser.name || localUser.email !== nextUser.email || state.meta?.userId !== profile.id || state.page === "auth";
    ...
    async function fetchRemote() {
      ...
      const { data, error } = await client.from("lerna_snapshots").select("schema_version,app_state,ai_state,v23_state,payload_hash,device_id,updated_at").eq("user_id", user.id).maybeSingle();
      if (error) throw error;
      return data || null;
    }
    ...
    async function uploadLocal() {
      ...
      try {
        ...
        const { data, error } = await client.from("lerna_snapshots").upsert({
          user_id: user.id,
          ...
          device_id: deviceId(),
          updated_at: now
        }, { onConflict: "user_id" }).select("payload_hash,device_id,updated_at").single();
        if (error) throw error;
        writeMeta({
          userId: user.id,
          email: user.email || null,
    ...
    function backupLocal() {
      const backupKey = `lerna_cloud_local_backup_${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}`;
  ...
  `;
    ...
    ` : "";
      ...
      <div class="head">
        ...
        <button class="close" id="cloud-close" aria-label="${esc(t("close"))}">\xD7</button>
      </div>
      ${configBlock}
      ${authBlock}
      ${syncBlock}
    ...
    const AUTO_INTERVAL_MS = 5 * 60 * 1e3;
    function isAutoMode() {
    ...
    async function autoSyncQuiet({ pull = false } = {}) {
      ...
      try {
        ...
        try {
          console.warn("[lerna-cloud] auto sync error:", error?.message || error);
        } catch {
        }
      } finally {
